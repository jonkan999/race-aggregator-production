...
const trainingTypes = {
  easy: '{{ training_type.easy_run }}',
  tempo: '{{ training_type.tempo_run }}',
  intervals: '{{ training_type.intervals }}',
  long: '{{ training_type.long_run }}',
  rest: '{{ training_type.rest_day }}',
};
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (generateButton && form) {
    generateButton.addEventListener('click', (e) => {
      ...
      const raceDate = document.getElementById('race-date').value;
      if (!raceDate) {
        alert('{{ please_select_date }}'); // Add this to your translations
        return;
      }
